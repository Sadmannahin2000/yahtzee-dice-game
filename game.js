...
// Declare a constant (i.e., rollButton) set equal to document, getElementById, passing argument rollDiceButton
...
// Declare a constant (i.e., speculativeScoreTab) set equal to document.getElementsByClassName, passing argument speculative-score
...
function rollDie() {
	...
	// Declare variable amountToRoll, to store the number of dice to roll, set equal to (5 - array diceSelected, property length
	const amountToRoll = 5 - diceSelected.length;
...
function drawDiceOnTable() {
	...
	for (let i = 0; i < diceOnTable.length; i++) {
		...
		// 1.  current value of array diceOnTable
...
function selectDieFromTable() {
	...
	//   2.  Radix 10
	const diceValue = parseInt(this.getAttribute('die-value'), 10);
	...
	const position = diceOnTable.indexOf(diceValue);
...
// Write function drawSelectedDice to draw the selected die on selected dice tray, parameter list includes value, index
...
function removeDieSelection() {
	...
	const position = diceSelected.indexOf(diceValue);
...
// Write function drawDieOnTable, parameter list includes value, index
...
function updateDiceAnywhere() {
	...
	if (diceSelected.length > 0) {
		...
		diceAnywhere = diceOnTable.slice();
	}
}
